...
async def get_dashboard_assignments(
    ...
    search: Optional[str] = Query(
        None,
        description="คำค้นหาแบบกว้าง (ชื่อ / เลขบัตรประชาชน / เลขบัตร อสม.)",
    ),
    citizenId: Optional[str] = Query(
        None,
        max_length=13,
        description="ค้นหาเฉพาะเลขบัตรประชาชน (ครบ 13 หลัก = ตรงเป๊ะ, ไม่ครบ = ขึ้นต้นด้วย)",
    ),
    osmCode: Optional[str] = Query(
        None,
        max_length=50,
        description="ค้นหาเฉพาะเลขบัตร อสม.",
    ),
    firstName: Optional[str] = Query(
        None,
        max_length=100,
        description="ค้นหาเฉพาะชื่อ (แบบมีคำนี้อยู่ในชื่อ)",
    ),
